...
const items: MenuItem[] = [
  ...
  getItem("Subscriptions", "subscription", <AiOutlineFile />, []),
];
...
const Sidebar = ({ onChange }: IProps) => {
  ...
  return (
    <>
      ...
      <button
        onClick={() => logout?.("/")}
        className="flex items-center justify-center gap-2 h-10 w-full rounded-[20px] text-white font-medium text-[17px] bg-secondary hover:bg-primary"
      >
        <span>
          <IoMdLogOut />
        </span>
